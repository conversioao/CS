...
const Credits = () => {
  ...
  useEffect(() => {
    if (isConfirmationModalOpen) {
      const timer = setTimeout(() => setIsConfirmationModalOpen(false), 5000);
  ...
  const handleSubmitPayment = async () => {
    ...
    try {
      ...
      const { data: paymentData, error: paymentError } = await supabase.from('payments').insert({
        user_id: user.id, package_id: selectedPlan.id, amount: selectedPlan.price,
        credits_purchased: selectedPlan.credits_amount, payment_method: paymentMethod,
        proof_url: proofUrl, status: 'pending',
      }).select('id').single();

      if (paymentError || !paymentData) throw paymentError || new Error("Falha ao criar pagamento.");

      const { error: transactionError } = await supabase.from('credit_transactions').insert({
        user_id: user.id, transaction_type: 'purchase', amount: selectedPlan.credits_amount,
        description: `Compra pendente: ${selectedPlan.name}`, related_payment_id: paymentData.id,
      });
      ...
      setPaymentMethod(""); setPaymentProof(null); setPhoneNumber("");
    } catch (error) {
      toast.error("Erro ao processar pagamento", { description: "Ocorreu um problema. Tente novamente." });
    } finally {
